...
if (isset($_POST['education']) && is_array($_POST['education'])) {
    ...
    foreach ($_POST['education'] as $edu) {
        ...
        // Only update start_year if it's provided
        $start_year = !empty($edu['start_year']) ? intval($edu['start_year']) : 0;
        $end_year = $current_study ? null : 
                (!empty($edu['end_year']) ? intval($edu['end_year']) : null);

        if (isset($edu['id']) && !empty($edu['id'])) {
            // Update existing education
            $update_edu_stmt->bind_param("siiiii", 
                $edu['school'], 
                $start_year, 
                $end_year,
                $current_study,
            ...
            $insert_edu_stmt->bind_param("isiii", 
                $resume_id,
                $edu['school'], 
                $start_year, 
                $end_year,
