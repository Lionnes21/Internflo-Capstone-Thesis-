<?php
   ...
   // Check if user is logged in - check both session variables
   ...
   // If logged in, fetch user details from the database
    if ($isLoggedIn) {
        // Use advisor_id if available, otherwise use user_id
        $userId = $_SESSION['advisor_id'] ?? $_SESSION['user_id'];
        
        $sql = 'SELECT first_name, middle_initial, last_name, suffix, email FROM m_advisors WHERE advisor_id = ?';
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('s', $userId);  // Changed to 's' if advisor_id is string
        $stmt->execute();
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send_message'])) {
        if (empty($sender_email)) {
            echo json_encode(['success' => false, 'error' => 'Not logged in or email not found']);
            exit();
        }
    ...
    $messages_query = "SELECT * FROM messaging_agreement WHERE recipient_email = ? ORDER BY timestamp DESC";
        ...
        <?php if ($messages->num_rows > 0): ?>
                <?php while($message = $messages->fetch_assoc()): ?>
                    <?php 
                        ...
                        $fileQuery = "SELECT file_path FROM messaging_agreement WHERE message_id = ?";
    ...
    <div class="date-banner">
        <p class="date-banner-text">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px" fill="#666666"><path d="M123.59-87.87q-33.79 0-58.39-24.61-24.61-24.61-24.61-58.39v-500.17h83v500.17H725.5v83H123.59Zm151.41-155q-32.06 0-55.74-23.53-23.67-23.53-23.67-55.64v-340.48q0-25.55 10.69-43.56 10.7-18.01 30.81-29.44L557.5-914.87l81.57 45.83-365.5 206.65 283.93 160.3 278.43-157.19q13.92-7.72 28.69-6.98 14.77.74 27.21 7.69 12.43 6.96 20.01 19.8 7.57 12.83 7.57 29.14v287.59q0 32.11-23.67 55.64-23.68 23.53-55.74 23.53H275Zm281.7-336.96-102-102 51-51 51 51 143-142 51 51-194 193Z"/></svg>
                <span class="text"><span style="font-weight: 600;">Internflo</span> Secure and Reliable Email Infrastructure for Professionals - <span style="font-weight: 600;">University of Caloocan City</span> Internflo©2025 </span>
            </span>
        </p>
    </div>
    ...
    <div id="fileDetailsModal" class="file-details-modal">
        <div class="file-details-modal-content">
            ...
            <div id="modal-content-container">
                <h2>Company Review for Commission on Audit</h2>
                <p class="file-details-info">This document provides details about the company that has submitted an account for approval.</p>
                
                <form id="companyDetailsForm">
                    <div class="file-details-section documents-section">
                        ...
                        <div class="documents-container">
                            <div class="documents-column">
                                <div class="detail-row">
                                    ...
                                    <input style="display: none;" type="text" id="bir-input" name="bir_registration" class="detail-input" placeholder="BIR document path">
                                </div>
                                <div class="detail-row">
                                    <span class="detail-label">CERTIFICATE OF REGISTRATION:</span>
                                    <span id="cor-document" class="detail-values"></span>
                                    <input style="display: none;" type="text" id="cor-input" name="certificate_of_registration" class="detail-input" placeholder="Certificate of Registration path">
                                </div>
                            </div>
                            <div class="documents-column">
                                <div class="detail-row">
                                    ...
                                    <input style="display: none;" type="text" id="permit-input" name="business_permit" class="detail-input" placeholder="Business Permit path">
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="file-details-columns">
                        <div class="file-details-column">
                            <h3>Company Details:</h3>
                            <div class="detail-row">
                                <span class="detail-label">Name:</span>
                                <span id="company-name" class="detail-value"></span>
                                <input style="display: none;" type="text" id="company-name-input" name="company_name" class="detail-input">
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">Industry:</span>
                                <span id="company-industry" class="detail-value"></span>
                                <input style="display: none;" type="text" id="company-industry-input" name="company_industry" class="detail-input">
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">Address:</span>
                                <span id="company-address" class="detail-value"></span>
                                <input style="display: none;" type="text" id="company-address-input" name="company_address" class="detail-input">
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">Phone:</span>
                                <span id="company-phone" class="detail-value"></span>
                                <input style="display: none;" type="text" id="company-phone-input" name="company_phone" class="detail-input">
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">Company Website:</span>
                                <span id="company-website" class="detail-value"></span>
                                <input style="display: none;" type="text" id="company-website-input" name="company_email" class="detail-input">
                            </div>
                        </div>
                        ...
                        <div class="file-details-column">
                            ...
                            <div class="detail-row">
                                ...
                                <input style="display: none;" type="email" id="recruiter-email-input" name="recruiter_email" class="detail-input">
                            </div>
                            <div class="detail-row">
                                <span class="detail-label">Contact Number:</span>
                                <span id="recruiter-contact" class="detail-value"></span>
                                <input style="display: none;" type="text" id="recruiter-contact-input" name="recruiter_mobile" class="detail-input">
                            </div>
                        </div>
                    </div>
                    
                    <div class="file-details-section">
                        <h2>Affiliate Approval</h2>
                        <p>Practicum coordinator will download the agreement document template and upload an e-sign</p>
            ...
            fetch(`get_file_details.php?message_id=${messageId}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('agreement-file').innerHTML = agreementDoc ? 
                        `<a href="../../admin/${agreementDoc}" target="_blank" class="document-link">${documentIcon} ${processFilePath(agreementDoc)}</a>` : 'N/A';
                })
                .catch(error => {
