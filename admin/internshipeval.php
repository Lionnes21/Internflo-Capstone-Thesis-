...
<head>
    ...
    <title>OnTheGo Admin - Internship Requests</title>
...
<section id="sidebar">
    ...
    <ul class="side-menu">
        ...
        <li><a href="#"><i class='bx bxs-user-detail icon'></i>Accounts</a></li>
        <li><a href="#"><i class='bx bxs-message-rounded-detail icon'></i>Feedbacks</a></li>
        <li>
        ...
        <li><a href="#"><i class='bx bxs-map icon'></i>Interns</a></li>
        <li><a href="internshipeval.php" class="active"><i class='bx bx-run icon'></i>Companies</a></li>
        <li>
                            ...
                            <table>
                                ...
                                <tbody>
                                    ...
                                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search_query'])) {
                                        ...
                                        $searchQuery = "SELECT * FROM recruiters WHERE 
                                        ...
                                        $result = $conn->query("SELECT * FROM recruiters");
                                    }
...
<td>
    <div class="action-buttons">
        ...
        <button class="forward-btn" onclick="showForwardForm(' . $row['id'] . ')">Forward</button>
    </div>
</td>
                                                    </tr>';
                                            }
                                        } else {
                                            echo '<tr><td colspan="4"><p class="not-found">No recruiter requests found</p></td></tr>';
                                        }
                                    } else {
                                        echo '<tr><td colspan="4"><p class="error">Error executing query</p></td></tr>';
                                    }
                                    ?>
                                </tbody>
                            </table>
...
<script>
    // Function to show the forward form below the table
function showForwardForm(id) {
    // Create the form HTML
    const formHTML = `
        <div class="forward-form">
            <h2>Forward Request</h2>
            <form id="forwardForm" method="post" action="forward_action.php" enctype="multipart/form-data">
                <input type="hidden" id="forwardRecruiterId" name="recruiter_id" value="${id}">
                <div>
                    <label for="fileInput">File:</label>
                    <input type="file" id="fileInput" name="file" required>
                </div>
                <div>
                    <label for="selectInput">Select Recipient:</label>
                    <select id="selectInput" name="recipient" required>
                        <option value="Roger">Roger</option>
                        <option value="Jespher">Jespher</option>
                        <option value="Mary">Mary</option>
                    </select>
                </div>
                <button type="submit">Submit</button>
                <button type="button" onclick="hideForwardForm()">Cancel</button>
            </form>
        </div>
    `;

    // Insert the form into the container
    document.getElementById("forwardFormContainer").innerHTML = formHTML;
}

// Function to hide the forward form
function hideForwardForm() {
    document.getElementById("forwardFormContainer").innerHTML = "";
}
</script>
<script src="js/script.js"></script>
<script>
    /// Get the modal
    var modal = document.getElementById("recruiterModal");
    ...
    function viewRecruiter(id) {
        fetch(`internshipeval.php?id=${id}`)
            .then(response => response.json())
            .then(data => {
                let content = `
                    <div class="company-logo" style="text-align: center;">
                        <img class="circular-logo" src="../RECRUITER/${data.company_logo}" alt="${data.company_name} Logo" style="max-width: 100px;">
                    </div>
                    <h1 class="company-name" style="text-align: center;">${data.company_name}</h1>
                    <p class="company-details" style="text-align: center;">${data.industry}</p>
                    <p class="company-details" style="text-align: center;">${data.company_address}</p>
                    <p class="company-details" style="text-align: center;">${data.company_phone}</p>
                    <p class="company-details" style="text-align: center;">${data.company_email}</p>
                    <p class="company-details" style="text-align: center;">${data.company_overview}</p>
                    
                    <h2 id="modalTitle" class="company-title" >Recruiter Details</h2>    
                    <p><strong>Full Name:</strong> ${data.first_name} ${data.middle_name || ''} ${data.last_name} ${data.suffix || ''}</p>
                    <p><strong>Email:</strong> ${data.email}</p>
                    <p><strong>Mobile Number:</strong> ${data.mobile_number}</p>
                    <p><strong>Documents:</strong></p>
                    <ul>
                        <li>Certificate of Registration: <a href="../RECRUITER/${data.certificate_of_registration}" target="_blank">View</a></li>
                        <li>BIR Registration: <a href="../RECRUITER/${data.bir_registration}" target="_blank">View</a></li>
                        <li>Business Permit: <a href="../RECRUITER/${data.business_permit}" target="_blank">View</a></li>
                    </ul>
                    <div style="text-align: center; margin-top: 20px;">
                        <button onclick="approveRecruiter(${data.id})" style="background-color: #4CAF50; color: white; padding: 8px 15px 10px 15px; margin-right: 5px; border: none; border-radius: 5px; cursor: pointer;">Approve</button>
                        <button onclick="declineRecruiter(${data.id})" style="background-color: #f44336; color: white; padding: 8px 15px 10px 15px; border: none; border-radius: 5px; cursor: pointer;">Decline</button>
                    </div>
                `;
    ...
    function approveRecruiter(id) {
        if (confirm('Are you sure you want to approve this recruiter?')) {
            ...
            .catch(error => {
                console.error('Error:', error);
                alert('An error occurred while approving the recruiter');
            });
        }
    }
