<?php
    ...
    $query = "SELECT student_id, firstname, lastname, middlename, suffix, course, year, section FROM students_clas WHERE 1=1";
    $count_query = "SELECT COUNT(*) as total FROM students_clas WHERE 1=1";
    ...
    if(isset($_POST['action']) && $_POST['action'] == 'update_student') {
        ...
        $update_stmt = $conn->prepare("UPDATE students_clas SET firstname = ?, lastname = ?, middlename = ?, suffix = ?, course = ?, year = ?, section = ? WHERE student_id = ?");
    ...
    if(isset($_POST['action']) && $_POST['action'] == 'get_student') {
        ...
        $student_stmt = $conn->prepare("SELECT student_id, firstname, lastname, middlename, suffix, course, year, section FROM students_clas WHERE student_id = ?");
    ...
    if(isset($_POST['action']) && $_POST['action'] == 'add_student') {
        ...
        $insert_stmt = $conn->prepare("INSERT INTO students_clas (student_id, firstname, lastname, middlename, suffix, course, year, section) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if(isset($_POST['action']) && $_POST['action'] == 'check_student_id') {
        ...
        $check_stmt = $conn->prepare("SELECT student_id FROM students_clas WHERE student_id = ?");
...
<section id="content">
    ...
    <main>
        <h1 class="title">COLLEGE OF LIBERAL ARTS AND SCIENCES</h1>
