...
<main>
    ...
    <div class="data">
        <div class="content-data">
            <div class="head">
                <h3>OnTheGo Completed Transactions</h3>
            </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                                ...
                                $searchQuery = "SELECT *, CONCAT(accounts.fname, ' ', accounts.lname, ' ', accounts.suffix) AS name FROM transactions
                					INNER JOIN accounts ON accounts.id = transactions.user_id WHERE transactions.id LIKE '%$search%' OR 
									CONCAT(accounts.fname, ' ', accounts.lname, ' ', accounts.suffix) LIKE '%$search%' OR accounts.username LIKE '%$search%'";
                                    ...
                                    if ($searchResult->num_rows > 0) {
                                        while ($searchRow = $searchResult->fetch_assoc()) {
                                            echo '<tr>
                                                ...
                                                <td>' . $searchRow['date'] . '</td>
                                                <td>' . $searchRow['details'] . '</td>
                                                </tr>';
                                        }
                                    } else {
                                ...
                                $qry = $conn->query("SELECT *, CONCAT(accounts.fname, ' ', accounts.lname, ' ', accounts.suffix) AS name FROM transactions
									inner join accounts on accounts.id = transactions.user_id");
